  ...
  return (
    <Listbox value={value} onChange={onChange} disabled={disabled}>
      <div className="relative">
        <ListboxButton className={`w-full flex justify-between items-center border border-border rounded px-3 py-2 outline-none focus:border-secondary ${disabled ? 'opacity-40 cursor-default' : 'cursor-pointer'}`}>
        ...
        <ListboxOptions className="absolute z-10 mt-1 w-full border border-border rounded shadow-sm">
          {options.map(o => (
            <ListboxOption
              key={o.value}
              value={o.value}
